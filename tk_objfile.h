...
int TKimpl_isFloatChar( char ch )
{
    if (ch == 'e')
        int a = 1;
    if ( ((ch>='0')&&(ch<='9')) || (ch=='-') || (ch=='.')||(ch=='e')||(ch=='n')||(ch=='a')) return 1;
    else return 0;
}
    ...
    while (line - start < objFileSize)
    {
        ...
        if (line[0]!='#')
        {
            ...
            while (token)
            {
                ...
                else if (TKimpl_compareToken("g", token, endtoken)) {
...
void TK_ParseObj( void *objFileData, size_t objFileSize, TK_ObjDelegate *objDelegate )
{
    ...
    for (int i = 0; i < numUniqueMtls; i++) {
        ...
        geom->materials[i].numTriangles = 0;// uniqueMtls[i].numTriangles;
    }
